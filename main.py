...
class TCUAdvisor(MDApp):
    ...
    def message_probability(self, command, recognised_words, single_response=False, required_words=[]):
        message_certainty = 0
    ...
    def response(self, *args):
        ...
        for x in botresponse.keys():
            if botresponse[x]['Command'] != command:
                ...
                response = f"Doesn't recognize!, We recommend you to call TCU counselor {counselor_number}"
                screen.get_screen('Message-screen').chat_list.add_widget(Response(text=response, size_hint_x=.75))
                self.speak_female(response)
                return
        ...
        for i in result.keys():
            if result[i]['Student ID'] == stud_id:
                if result[i]['Password'] == password:
                    ...
                    self.speak(f"Welcome to TCU newest appliation,{(result[i]['Name'])}!")

                    screen.get_screen('Profile-screen').add_widget(User_data(text=result[i]['Name'], font_name="fonts/Poppins.otf", font_size=25, halign="center",pos_hint={"center_y": .6}))
                    screen.get_screen('Profile-screen').add_widget(User_data(text=result[i]['Student ID'], pos_hint={"center_x": 1.1, "center_y": .47}))
                    screen.get_screen('Profile-screen').add_widget(User_data(text=result[i]['Year & Course'], pos_hint={"center_x": 1.1, "center_y": .42}))
                    screen.get_screen('Profile-screen').add_widget(User_data(text=result[i]['Section'], pos_hint={"center_x": 1.1, "center_y": .37}))
                    return
